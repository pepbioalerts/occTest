...
The outputs of occurrenceTests is a  a dataframe with coordinates, and a number of additional columns that inform of tests, corrections and summary measures of tests grouped by kinds of tests (e.g. scores, see below).
...
errorAcceptance | majority (error accepted up to 50% of the tests), relaxed (70%), stringent (20%), custom . Level of error you accept in the groupings| character. Deffault is majority
errorThreshold  | if errorAcceptance set to custom. The threshold of error accepted by group.  | double. 0 to 1


```{r}
outDougFirSelec = occFilter(df = outDougFir,level = 2) 
...
check how the groupings changed with different levels and differnt threshold accpetance levels 


```{r}
outDougFirSelec2 = occFilter(df = outDougFir, level = 2, errorAcceptance = 'stringent') 
...
We will apply *occurrenceTests* to a dataset that has more fields and for that we need to specify them.
