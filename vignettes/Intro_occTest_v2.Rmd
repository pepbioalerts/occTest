...
The outputs of occTest is a  a dataframe with coordinates, and a number of additional columns that inform of tests, corrections and summary measures of tests grouped by kinds of tests.
...
```{r}
outDougFirSelecStringent = occFilter (df = outDougFir, errorAcceptance = 'strict')
...
We will apply *occTest* to a dataset that has more information like the locality names, or the accuracy of the coordinates. For that we need to specify that firest before running the function.
...
```
Now, flter the results with a rule based on majority and grouped based on 
