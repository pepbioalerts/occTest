...
plot.occTest<-function(x,occFilter_list=NULL,show_plot=F,...){
  ...
  if(!is.null(occFilter_list)){
    ...
    filter_occ<-function(score,testName){
      current_treshold<-errorRule %>% dplyr::filter(test == str_remove(testName,"_score")) %>% dplyr::pull(errorThreshold)
...
printOccTest<-function(x,occFilter_list=NULL,show_plot=F){
