...
plot.occTest<-function(x,occFilter_list=NULL,show_plot=FALSE,...){
  ...
  ## this dataframe contains ecery occurrences, useful for the fist map displaying the first filtering
  ...
  list_of_ggplot[[1]]<-ggplot(full_dataset_sf)+theme_bw()+geom_sf(data=countries_natural_earth,color="grey60",fill="grey98")+
    ...
    labs(color="Filter reason",title="Coordinates filtering",subtitle =paste0( "occurrences without coordinates = ",n_coords_missing ,"\noccurrences filtered =  ",n_coords_filtered))+
  ...
  if(!is.null(occFilter_list)){
    ...
    if(nrow(filtered_dataset)==nrow(full_dataset)) message ("occFilter filtered 0 occurrences")
    ...
    ## This dataframe (transformed to an sf object) contain the filtered occurrences, and which tests they passed or not
    ...
    plot_a_test_results<-function(score_name){
      ...
      filtered_dataset_scores_sf$occ_state<-ifelse(filtered_dataset_scores[,bool_score_name],"Removed",ifelse(filtered_dataset_scores$filtered,"Removed by another test","Kept occurrences"))
      ...
      plot_result<-ggplot(filtered_dataset_scores_sf)+theme_bw()+geom_sf(data=countries_natural_earth,color="grey60",fill="grey98")+
        ...
        labs(color="Filter results",title=title_char,subtitle =ifelse(n_occ_filtered==0,"No occurrences filtered by these tests",paste0(n_occ_filtered," occurrences removed by these tests")))+
        ...
        scale_color_manual(breaks =c("Removed","Removed by another test","Kept occurrences"),values=c("brown","goldenrod","grey45") )
